from peewee import SqliteDatabase, Model, CharField, IntegerField, ForeignKeyField, TextField

from database.movie_data import MovieData

db = SqliteDatabase("history.db")
cache_db = SqliteDatabase("cache.db")
	...
	def save_search(cls, message, movie: MovieData, category: str):
		try:
			...
			try:
				...
				Movies.create(user=user, link=movie.link, movie_name=movie.name, year=movie.year, category=category)
			except:
